...
export const TechniciansPage = () => {
    ...
    const initialFormState: Technician = TechnicianGestor.Construct('','')

    const { control, register, handleSubmit, reset, formState: {errors}} = useForm<Technician>({defaultValues:{...initialFormState}})
    ...
    const handleEditButton = (Technician: Technician) => {
        setToUpdate()
        reset(Technician)
        toggleModal()
    }

    const handleDeleteButton = async (Technician: Technician) => {
        ...
        if (confirm) {
            try {
                const response = await TechnicianGestor.Delete(Technician)
    ...
    const save = async (data: Technician) => {
        console.log(data)
        try {
            let response
            if (isCreating) response = await TechnicianGestor.Create(data)
            if (isUpdating) response = await TechnicianGestor.Update(data)
