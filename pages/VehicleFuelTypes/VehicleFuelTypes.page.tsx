...
export const VehicleFuelTypesPage = () => {
    
    const initialFormState: VehicleFuelType = VehicleFuelTypeGestor.Construct('','')

    const { control, register, handleSubmit, reset, formState: {errors}} = useForm<VehicleFuelType>({defaultValues:{...initialFormState}})
    const { isCreating, isUpdating, setToCreate, setToUpdate} = useCrud()
    const { isModalVisible, toggleModal} = useModal()

    const [ list, setList] = useState<VehicleFuelType[]>([])
    ...
    const handleEditButton = (VehicleFuelType: VehicleFuelType) => {
        setToUpdate()
        reset(VehicleFuelType)
        toggleModal()
    }

    const handleDeleteButton = async (VehicleFuelType: VehicleFuelType) => {
        ...
        if (confirm) {
            try {
                const response = await VehicleFuelTypeGestor.Delete(VehicleFuelType)
    ...
    const save = async (data: VehicleFuelType) => {
        try {
            let response
            if (isCreating) response = await VehicleFuelTypeGestor.Create(data)
            if (isUpdating) response = await VehicleFuelTypeGestor.Update(data)
...
export default VehicleFuelTypesPage