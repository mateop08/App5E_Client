...
export const VehiclePlateTypesPage = () => {
    
    const initialFormState: VehiclePlateType = VehiclePlateTypeGestor.Construct('','')

    const { control, register, handleSubmit, reset, formState: {errors}} = useForm<VehiclePlateType>({defaultValues:{...initialFormState}})
    const { isCreating, isUpdating, setToCreate, setToUpdate} = useCrud()
    const { isModalVisible, toggleModal} = useModal()

    const [ list, setList] = useState<VehiclePlateType[]>([])
    ...
    const handleEditButton = (VehiclePlateType: VehiclePlateType) => {
        setToUpdate()
        reset(VehiclePlateType)
        toggleModal()
    }

    const handleDeleteButton = async (VehiclePlateType: VehiclePlateType) => {
        ...
        if (confirm) {
            try {
                const response = await VehiclePlateTypeGestor.Delete(VehiclePlateType)
    ...
    const save = async (data: VehiclePlateType) => {
        console.log(data)
        try {
            let response
            if (isCreating) response = await VehiclePlateTypeGestor.Create(data)
            if (isUpdating) response = await VehiclePlateTypeGestor.Update(data)
...
export default VehiclePlateTypesPage